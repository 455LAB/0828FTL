...
void RemapBadBlock()
{
	...
	for(blockNo=0 ; blockNo<USER_BLOCKS_PER_LUN ; blockNo++)
	{
		for(dieNo=0 ; dieNo<USER_DIES ; dieNo++)
		{
			//zheng
			//lun0
			//xil_printf("bad or good: %d  \n\r",phyBlockMapPtr->phyBlock[dieNo][blockNo].bad);
...
void RecoverBadBlockTable(unsigned int tempBufAddr)
{
	...
	//original bbt info stored in the first page of each block. zheng
	//size : USER_DIES * USED_PAGES_FOR_BAD_BLOCK_TABLE_PER_DIE * tempBbtBufEntrySize;
	...
	tempReadBufEntrySize = BYTES_PER_NAND_ROW;//row size is equal to total page size zheng

	for(dieNo = 0; dieNo < USER_DIES; dieNo++)
	{
		//store the certain  BBT address of each die
